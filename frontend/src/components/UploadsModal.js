...
import { fileUpload, updateFile, deleteFile } from "../Services/FileService";
export const UploadsModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      if (initialData) {
        const response = await updateFile(filesDetail.id, filesDetail);
        if (response) {
  ...
  const handleDeleteFile = async () => {
    try {
      const response = await deleteFile(initialData.id);
      if (response) {
